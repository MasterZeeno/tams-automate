...
async function scrapeTable(page, tableURL, tbl_name) {
    try {
        let siteURL;
        if (tableURL === 'attendance') {
            siteURL = `${process.env.TAMS_BASE_URL}/${tableURL}`;
        else
            siteURL = `${process.env.TAMS_BASE_URL}/filing/${tableURL}`;
        }
    ...
    const tableData = await page.evaluate((tbl_name) => {
        ...
        const headers = Array.from(table.querySelectorAll('th'))
            ...
            .filter(header => header !== 'Action' && header !== 'Attachment');
